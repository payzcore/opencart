...
class ControllerExtensionPaymentPayzcore extends Controller {
     ...
     * Renders the chain/token selector (if multiple chains enabled) and confirm button.
     *
    ...
    public function index() {
        ...
        $enabled_chains = $this->model_extension_payment_payzcore->getEnabledChains();
        ...
        $chains = array();
        foreach ($enabled_chains as $c) {
            $chains[] = array(
                'code'  => $c,
                'label' => isset($chain_labels[$c]) ? $chain_labels[$c] : $c,
            );
        }

        $data['chains'] = $chains;

        // For single chain: show specific token and chain label
        if (count($enabled_chains) === 1) {
            $single_chain = $enabled_chains[0];
            $token = ($single_chain === 'TRC20') ? 'USDT' : $default_token;
            $data['chain_label'] = $this->getChainLabel($single_chain);
    ...
    public function confirm() {
        ...
        try {
            ...
            // Get enabled chains for validation
            $enabled_chains = $this->model_extension_payment_payzcore->getEnabledChains();

            // Read chain/token from POST (checkout form) or fall back to defaults
            $chain = isset($this->request->post['payzcore_chain']) ? $this->request->post['payzcore_chain'] : '';
            $default_token = $this->model_extension_payment_payzcore->getDefaultToken();
            $token = isset($this->request->post['payzcore_token']) ? $this->request->post['payzcore_token'] : $default_token;

            // Validate chain is in enabled list
            if (empty($chain) || !in_array($chain, $enabled_chains)) {
                $chain = $enabled_chains[0];
            }
            ...
            // If store currency is not USD, try to convert
            ...
            $this->model_checkout_order->addOrderHistory(
                $order_id,
                $pending_status_id,
                'Stablecoin payment created. Waiting for ' . $token_name . ' transfer on ' . $chain . '.',
    ...
    public function callback() {
        ...
        if (!Payzcore::verifyWebhookSignature($raw_body, $signature, $webhook_secret)) {
    ...
    private function renderPaymentPage($payment, $order_info) {
        ...
        // Chain display
        $data['chain_label'] = $this->getChainLabel($payment['chain']);
        ...
        $data['explorer_url'] = $this->getExplorerUrl($payment['chain'], $payment['address']);
    ...
    /**
     * Get a human-readable label for a blockchain chain code.
     *
     * @param string $chain Chain code (TRC20, BEP20, ERC20, POLYGON, ARBITRUM).
     *
     * @return string Human-readable chain label.
     */
    private function getChainLabel($chain) {
        $this->load->language('extension/payment/payzcore');
        ...
        return isset($labels[$chain]) ? $labels[$chain] : $chain;
    }

    /**
     * Get the blockchain explorer URL for an address on a given chain.
     *
     ...
     */
    private function getExplorerUrl($chain, $address) {
        $explorers = array(
        ...
        $base = isset($explorers[$chain]) ? $explorers[$chain] : 'https://tronscan.org/#/address/';
