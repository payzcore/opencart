...
class ModelExtensionPaymentPayzcore extends Model {
    ...
    public function install() {
        ...
        // v1.0.0 migration: add token column + widen chain column for existing installs
        ...
        if (!$columns->num_rows) {
            $this->db->query(
                "ALTER TABLE `" . DB_PREFIX . "payzcore_payments`
                 ADD COLUMN `token` VARCHAR(10) NOT NULL DEFAULT 'USDT' AFTER `chain`"
            );
        }

        $chain_col = $this->db->query(
            "SHOW COLUMNS FROM `" . DB_PREFIX . "payzcore_payments` WHERE Field = 'chain'"
        );

        if ($chain_col->num_rows && strpos($chain_col->row['Type'], '16') === false) {
            $this->db->query(
                "ALTER TABLE `" . DB_PREFIX . "payzcore_payments`
                 MODIFY COLUMN `chain` VARCHAR(16) NOT NULL DEFAULT ''"
            );
        }
