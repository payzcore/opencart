...
class ControllerExtensionPaymentPayzcore extends Controller {
    ...
    public function index() {
        ...
        $data['cached_chains'] = array();

        if (!empty($cached_json)) {
            $parsed = json_decode($cached_json, true);
            if (is_array($parsed) && !empty($parsed['chains'])) {
                $data['cached_chains'] = $parsed['chains'];
            }
        }
        ...
        $lang_keys = array(
            ...
            'text_last_sync', 'text_available_chains', 'text_chains_help',
    ...
    private function fetchAndCacheConfig($api_url, $api_key) {
        $url = rtrim($api_url, '/') . '/api/v1/config';
        ...
        curl_setopt_array($ch, array(
            ...
            CURLOPT_HTTPHEADER     => array(
                ...
                'User-Agent: payzcore-opencart/1.1.1',
            ),
